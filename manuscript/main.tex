...
The LLM-automated coding from translated English transcripts was moderately correlated with the human coding (Pearson's $\rho$ = 0.53; OLS $r^{2}$ = 0.28), with a 10 p.p.\ increase in the human-coded essential diagnostic checklist predicting a 5.9 p.p.\ increase in the LLM-coded checklist (\textbf{Figure \ref{fig:validation}}, Panel C). When the LLM coded directly from the original Vietnamese transcripts -- without a separate translation step -- performance was comparable (Pearson's $\rho$ = 0.51; OLS $r^{2}$ = 0.26; \textbf{Figure \ref{fig:validation}}, Panel D), suggesting that translation is not a binding constraint for multilingual deployment.
