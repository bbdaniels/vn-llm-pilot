#!/usr/bin/env python3
"""classify_conf_vn.py -- Re-classify Vietnamese transcripts with confidence scores.

Same pipeline as classify_vn.py but asks for confidence (0.0-1.0) per item.
Outputs confidence columns (suffixed _conf) alongside binary scores.

Outputs:
  - data/pilot-coded-conf-vn.dta
  - data/pilot-coded-conf-vn.json
"""
...
OUTPUT_DTA = DATA_DIR / "pilot-coded-conf-vn.dta"
OUTPUT_JSON = DATA_DIR / "pilot-coded-conf-vn.json"
...
def main():
    ...
    print(f"\nClassifying {N} observations (Vietnamese, with confidence)...\n")
